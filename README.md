...
<p align="center">
  ...
  <br>
  <sub>Discord下旨 → 太子分拣 → 中书省规划 → 门下省审议 → 六部并行执行 → 奏折回报（30 秒）</sub>
</p>
</details>

> 🐳 **没有 OpenClaw？** 跑一行 `docker run -p 19527:19527 cft0808/edict` 即可体验完整看板 Demo（预置模拟数据）。
...
```bash
docker run -p 19527:19527 cft0808/edict
```
打开 http://localhost:19527 即可体验军机处看板。
